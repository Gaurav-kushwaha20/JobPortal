...
exports.UserSighUp = async (req, res) => {
    try {
        ...
        // send token in mail
        const verificationUrl = `${process.env.FRONTEND_DOMAIN}/user/verify-email/${generatedToken}`;
...
exports.verifyUser = async (req, res) => {
    ...
    try {
        ...
        if (!user) {
            ...
            return res.status(400).json({success: "User verified successfully, processed to login!"})
        }
...
exports.sendTokenToResetPassword = async (req, res) => {
    ...
    const verificationUrl = `${process.env.FRONTEND_DOMAIN}/user/verify-password-reset-token/${token.token}`
    // html content for beautiful UI in mail
